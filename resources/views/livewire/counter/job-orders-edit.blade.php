...
new class extends Component {
    ...
    public function mount(JobOrder $jobOrder)
    {
        ...
        $this->customer_name = $jobOrder->customer_name ?? '';
        $this->customer_email = $jobOrder->customer_email ?? '';
        $this->customer_phone = $jobOrder->customer_phone ?? '';
        $this->customer_address = $jobOrder->customer_address ?? '';
        $this->device_brand = $jobOrder->device_brand ?? '';
        $this->device_model = $jobOrder->device_model ?? '';
        $this->serial_number = $jobOrder->serial_number ?? '';
        $this->issue_description = $jobOrder->issue_description ?? '';
    ...
    public function save(): void
    {
        ...
        // Update status to assigned if a technician was just assigned
        if ($validated['assigned_to'] && !$this->jobOrder->assigned_to && $this->jobOrder->status === JobOrderStatus::PENDING) {
            $validated['status'] = JobOrderStatus::ASSIGNED;
        }
    ...
    <div class="lg:col-span-3">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
            <div class="bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-3">
                ...
                <p class="text-sm text-blue-100 mt-1">{{ $jobOrder->job_order_number }}</p>
            </div>
            ...
            <div class="p-4 space-y-4">
                ...
                <!-- Total -->
                <div class="pt-2">
                    <div class="flex justify-between items-center p-3 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg">
                        <span class="text-sm font-bold text-zinc-900 dark:text-white uppercase">Estimated Total</span>
                        <span class="text-2xl font-bold text-blue-600 dark:text-blue-400">₱{{ number_format($this->estimated_cost, 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="lg:col-span-7 flex-1 min-w-0">
        <form wire:submit="save" class="space-y-4">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 overflow-hidden px-4 py-3">
                ...
                <input type="text" value="{{ $jobOrder->job_order_number }}" disabled class="w-full px-3 py-2 text-sm border border-zinc-200 dark:border-zinc-700 rounded-lg bg-zinc-50 dark:bg-zinc-900 text-zinc-900 dark:text-white">
            </div>
