...
new class extends Component {
    ...
    public function mount(User $user): void
    {
        ...
        // Load existing skillsets for technicians
    ...
    public function save(): void
    {
        $rules = [
            ...
            'email' => ['required', 'string', 'email', 'max:255', ValidationRule::unique('users')->ignore($this->user->id)],
        ...
        $rules['skillsets'] = ['array'];
        $rules['skillsets.*.name'] = ['nullable', 'string', 'max:255'];
        $rules['skillsets.*.description'] = ['nullable', 'string'];
        $rules['skillsets.*.years_experience'] = ['nullable', 'integer', 'min:0'];
        $rules['skillsets.*.certifications'] = ['nullable', 'string'];
        $rules['skillsets.*.is_primary'] = ['nullable', 'boolean'];
        ...
        // Sync skillsets for technicians
        if ($this->role === Role::TECHNICIAN->value) {
            // simple approach: delete existing and recreate from input
            $this->user->skillsets()->delete();
            ...
            // remove skillsets if role changed away from technician
            $this->user->skillsets()->delete();
            $this->skillsets = [];
        }
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-4 mb-6">
                ...
                <div class="flex-1">
                    <h1 class="text-3xl font-bold bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent">Edit Staff Member</h1>
                    <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">Update staff member information, role, and security settings</p>
                </div>
            </div>
        </div>
            ...
            <form wire:submit="save" class="p-6 space-y-6">
                ...
                <div class="pt-6 border-t border-zinc-200 dark:border-zinc-700">
                    <div class="mb-4">
                        ...
                        <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1">Leave blank to keep the current password unchanged</p>
                    </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-zinc-200 dark:border-zinc-700">
                    <a href="{{ route('staff.index') }}" wire:navigate class="px-5 py-2.5 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-700 dark:hover:bg-zinc-600 text-zinc-700 dark:text-zinc-200 font-semibold rounded-xl transition-all shadow-sm hover:shadow">
                        Cancel
                    </a>
